...
def main():
	...
	MobileNet_with_imagenet_full_path = os.path.join(args.output_dir, 'MobileNet_with_ImageNet_full.h5') 
	tf.keras.models.save_model(full_model, MobileNet_with_imagenet_full_path)
	print('Successfully saved full model to {}'.format(MobileNet_with_imagenet_full_path))
	...
	MobileNet_quant_path = os.path.join(args.output_dir, 'MobileNet_with_ImageNet_quant.tflite')
	open(MobileNet_quant_path, 'wb').write(quant_model)	
	print('Successfully saved the quantized model to {}'.format(MobileNet_quant_path))

	# 사전에 edgetpu-compiler를 install하여야 함
	os.system('edgetpu_compiler {0}'.format(MobileNet_quant_path))
